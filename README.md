...
**Демо-данные для большого экрана в UI (только PostgreSQL):** после **`pnpm db:push`** и при необходимости **`pnpm --filter @birzha/api db:reset-test-data`** выполните **`pnpm --filter @birzha/api db:seed-demo`** из корня монорепо (или `cd apps/api && pnpm db:seed-demo`). Появятся три закупочные накладные с префиксом номера **`DEMO-`**, три рейса, три продавца (`demo-seed-seller-1..3`), отгрузки и продажи. Повторный запуск без очистки завершится ошибкой; пароль продавцов — **`BIRZHA_DEMO_SEED_PASSWORD`** (не короче 10 символов). Не использовать на продакшене с реальными данными без резервной копии БД.
