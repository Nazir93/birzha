...
Схема (таблицы) **не** удаляется. Очищаются накладные, партии, рейс, продажи/недостачи/отгрузки, `sync`, контрагенты, склады, калибры; **заново** вставляются склады и калибры **как в сиде** (Манас/Каякент, калибры с «Помидоры»). Учётные записи `users` / роли **не** трогаются.

**Перед** этим сделайте `pg_dump`, если на БД что-то ценное.

```bash
cd /opt/birzha/apps/api
# Скрипт читает `apps/api/.env` (нужен `DATABASE_URL` и доступ к БД)
...
Подробнее — комментарий в `apps/web/vite.config.ts`. После установки ярлык открывает `/s` (кабинет продавца). **Офлайн-очередь продаж не входит в PWA** — без сети API недоступен; SW кэширует только статику приложения.
...
6. В UI: баннер **«Установить»** (`PwaInstallBanner`) при поддержке браузером события `beforeinstallprompt`; на **iOS Safari** — текст подсказки «Поделиться → На экран Домой» (без системного диалога установки).
