...
- Партии: **`GET /batches`** (список; при PG — join **`nakladnaya`** + **`allocation`** в `batch-list-http.ts`); **распределение (шаг 3):** **`PATCH /batches/:batchId/allocation`** (только PG, `qualityTier` / `destination`); создание (`POST /batches`), оприходование, отгрузка в рейс, продажа с рейса, **фиксация недостачи** `POST /batches/:batchId/record-trip-shortage` (`tripId`, `kg`, `reason`).
...
- **Распределение (шаг 3):** вкладка **«Распределение»** (`/distribution`, панель `distribution` в `role-panels`); `PATCH /api/batches/:batchId/allocation` (только при PostgreSQL), в **`GET /batches`** — **`allocation`**: `qualityTier`, `destination` (см. `README.md`, `dlya-zakazchika-zapolnenie.md`). **Карта кода (приём → распределение):** [receive-and-distribution-code-map.md](architecture/processes/receive-and-distribution-code-map.md).
