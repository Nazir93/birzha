...
|-------------------|---------|-------|----------------|----------------|
| **admin** | `/a` | `/a` | Сводка админа (KPI + ссылки) | `/a/inventory`, `/a/users`, рабочие разделы `/a/*` |
| **manager** | `/o` | `/o/reports` или закупка товара | Отчёты / закуп по умолчанию | Панели `/o`; при входе как manager (без admin) доступ изолирован в `/o` — без `/a/*`, `/b/*`, `/s/*` |
...
| **seller** (только полевой) | `/s` | `/s` | Форма продажи с рейса (выбор рейса в форме при нескольких активных); **без** подразделов `/s/reports` и `/s/operations` в интерфейсе |
