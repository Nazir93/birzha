...
HTTP и use case: **`README.md`** (таблица API). Концептуальные блоки `purchaseDocuments` / `purchaseItems` в этом файле остаются ориентиром для расширения модели (поставщик как FK, фото, статусы документа) — пока часть полей перенесена в текстовые поля шапки или не реализована.
