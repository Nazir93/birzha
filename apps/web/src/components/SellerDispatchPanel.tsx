  ...
  return (
    <div role="region" aria-label="Отгрузка">
      ...
      <p className="birzha-callout-info" style={{ margin: "0 0 0.75rem" }}>
        Какой продавец закреплён за каким рейсом и какой товар на рейсе по данным отгрузки. У продавца в кабинете
        появятся строки для продажи только после того, как склад оформит <strong>отгрузку со склада в рейс</strong> по
        партиям (раздел «Операции» / «Рейсы»); одна привязка рейса к продавцу или погрузочная накладная без этой
        отгрузки кг в машину рейса не переносят.
      </p>
